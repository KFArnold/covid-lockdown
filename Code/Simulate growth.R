...
for (i in countries_eur) {
  ...
  date_start <- summary_eur_i %>% pull(Date_start)  # (starting date of simulation)
  date_end <- as.Date(date_start + max_t)  # (end date of simulation) 
...
rm(i, j, t, country, data_eur_i, summary_eur_i, knots_best_i,
   date_start, date_end, date_lockdown, dates, 
   daily_cases_sim_i, cumulative_cases_end_sim_i, 
