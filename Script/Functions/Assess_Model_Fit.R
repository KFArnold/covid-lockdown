...
#'
#' @examples
#' Assess_Model_Fit(country = "United Kingdom")
Assess_Model_Fit <- function(country) {
  ...
  diff_time_to_thresholds <- Calculate_Diff_Time_To_Thresholds(country = country) 
  ...
  diff_total_cases <- Calculate_Diff_Total_Cases(country = country)
  
  # Calculate the Poisson deviance between the observed data and simulated
  # natural history (both incident and cumulative cases)
  pois_dev_natural_history <- Calculate_Pois_Dev_Natural_History(country = country)
