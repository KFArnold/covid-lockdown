...
Summary_Table_Effects_Between_Country <- function(outcomes = c("Length_lockdown",
  ...
  Import_Unloaded_CSV_Files(filenames = "effects_between_countries",
                            silent = TRUE)
  ...
  effects_formatted <- effects_between_countries %>%
    mutate(across(c(Effect:R_squared), 
                  ~formatC(round(., digits = n_decimals), format = "f", digits = n_decimals)),
           Adjusted = ifelse(is.na(Covariates), "Unadjusted", "Adjusted"),
